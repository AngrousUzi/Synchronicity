...
def process_single_stock(args):
    """处理单个股票的函数，用于多进程"""
    full_code, start_tmp, end, workday_list, error_list, start, base_dir = args
    try:
        df_stock, workday_list, _ = get_data(start=start_tmp, end=end, exg=full_code[:2], 
...
def agg_raw(index_code,start,end,base_dir,cpu_parallel_num=5):
    exg=index_code[:2]
    ...
    if len(composites["full_code"]) > 0:
        ...
        # 准备多进程参数
        process_args = [(full_code, start_tmp, end, workday_list, error_list, start, base_dir) 
